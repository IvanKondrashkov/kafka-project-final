...
public class SchemaRegistryUtil {
    ...
    private static String readSchemaFromResources() {
        try (InputStream inputStream = SchemaRegistryUtil.class.getClassLoader().getResourceAsStream("event-schema.json")) {
            if (inputStream == null) {
                throw new FileNotFoundException("Not found event-schema.json");
            }
